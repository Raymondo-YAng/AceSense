...
function MetricCard({ label, value, delta, isPositive }) {
  return (
    <div className="flex min-w-[150px] flex-1 flex-col gap-2 rounded-lg p-6 border border-accent bg-card/20">
      ...
      <p className={`text-base font-medium leading-normal ${isPositive ? 'text-success' : 'text-danger'}`}>{delta}</p>
    </div>
  );
}
  ...
  return (
    <div className="flex gap-4 bg-background px-4 py-3 border-b border-card/50 last:border-0">
      ...
      <div className="flex flex-1 flex-col justify-center">
        ...
        <p className="text-secondary text-sm font-normal leading-normal opacity-80 mt-0.5">{desc}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-background pb-8">
      <div className="flex items-center bg-background p-4 pb-2 justify-between sticky top-0 z-10">
        ...
        <h2 className="text-white text-lg font-bold leading-tight tracking-[-0.015em] flex-1 text-center pr-12">Analysis Results</h2>
      </div>

      <h2 className="text-white text-[22px] font-bold leading-tight tracking-[-0.015em] px-4 pb-3 pt-5">Swing Metrics</h2>
      ...
      <h2 className="text-white text-[22px] font-bold leading-tight tracking-[-0.015em] px-4 pb-3 pt-5">{videoUrl ? 'Your Analysis Video' : 'Comparison to Pro'}</h2>
      <div className="p-4">
        {videoUrl ? (
          <div className="relative aspect-video rounded-lg overflow-hidden bg-card flex items-center justify-center">
            {isError ? (
              <div className="flex flex-col items-center gap-2 p-4 text-center">
                ...
                <p className="text-secondary text-sm">Please wait a few seconds and refresh the page.</p>
                <button 
        ...
        <div className="flex min-w-72 flex-1 flex-col gap-2">
          ...
          <div className="grid min-h-[180px] grid-flow-col gap-6 grid-rows-[1fr_auto] items-end justify-items-center px-3 pt-4">
            <div className="w-full flex flex-col justify-end h-full">
              <div className="bg-card border-t-2 border-secondary w-full transition-all duration-1000 ease-out" style={{ height: '70%' }} />
            </div>
            <div className="w-full flex flex-col justify-end h-full">
              <div className="bg-card border-t-2 border-secondary w-full transition-all duration-1000 ease-out" style={{ height: '40%' }} />
            </div>
      ...
      <h2 className="text-white text-[22px] font-bold leading-tight tracking-[-0.015em] px-4 pb-3 pt-5">Areas for Improvement</h2>
      ...
      <h2 className="text-white text-[22px] font-bold leading-tight tracking-[-0.015em] px-4 pb-3 pt-5">Suggested Exercises</h2>
      <div className="flex flex-col">
